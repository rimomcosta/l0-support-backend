There are two architectural plans at Adobe Commerce Cloud:
1.Started Plan - This is a cluster of 3 nodes but each node is just a docker container
2.Pro Plan - This is a cluster of 3 dedicated nodes, but the merchant can hire split architecture, which provides more nodes.
The services provided by Adobe Commerce Cloud are:
a. PHP-FPM.
b. Nginx, which is configurable through the file .magento.app.yaml file. Whenever possible, we prefer to use Fastly for external access issues, like blocking IPs, WAF and others.
c. MariaDB.
d. Redis.
e. RabbitMQ.
f. OpenSearch but some merchants opt for using our SaaS Livesearch.
g. GlusterFs.
h. Galera cluster.
i. Fastly CDN for caching and content delivery. Credentials at /mnt/shared/fastly_tokens.txt.
j. NewRelic with a dedicated dashboard called Observation Tool.
The volumes are write protected, and sudo commands are not allowed, and they are managed by the platform.sh.
- Changing configurations in env.php is a bad practice as it is auto-generated by ece-tools during the deployment. The configurations are set in ".magento.env.yaml", ".magento.app.yaml" which is base64 encoded into the env variable MAGENTO_CLOUD_APPLICATION during the deployment, ".magento/routes.yaml" and ".magento/services.yaml".
- The credentials are in the env variable MAGENTO_CLOUD_RELATIONSHIPS encoded in base64. 
- During the investigation, try to find correlations between all the data and information you have.

# Guidance for performance optimization
There are several issues that, at first glance, don't seem to be related, but when you do a deep analysis, the underlying problem is mostly the same: Bad Performance. The following suggestions should fix most of the issues related to:
- Outages
- Slowness
...
The recommendations below are a "copy-paste template" that can be used to send to the merchant. Based on your investigation, you should judge what is relevant to send to the merchant. Always cross the follow information with the server's data and only mention what you see in the server.
Be aware that each bullet point is a different recommendation. Don't copy and paste the suggestions blindly.
One problem alone won't take down a server, but a chain of small issues, when aligned, can bring it to its knees.
- Ready to copy and paste answers, copy what makes sense only:
#### Cache Flush
...
[note]If Redis's maxmemory usage is low, please ignore this suggestion.
[explanation]This explains why the Redis maxmemory limit is 10Gb:
...
[explanation]Redis L2 cache is an implementation that helps Magento to read the cache locally first and check the remote only if the local cache is not found. This drastically reduces the inter-node network traffic and improves the cache loading response time.
...
[alt]Add this line below if the merchant doesn't have much traffic and /dev/shm usage is low:
...
The configurations described above are not set by default when a server is provisioned because L2 cache is a new implementation.
This is how to enable it in .magento.env.yaml:
```
stage:
  deploy:
...
```
I don't like suggesting this because most merchants never re-enable it, and keeping it disabled can increase the load on the server if the cache is purged on the Fastly side. Fastly Soft Purge and Enable Stale Cache should also be recommended. This is just in situations where the Redis max memory usage is too high.
...
- Use the [Quality Patch Tool](https://experienceleague.adobe.com/tools/commerce-quality-patches/index.html) to apply the patch **MCLOUD-11329** to fix an issue where missed jobs unnecessarily wait for cron job locks, which can lead to lock contention and unnecessary resource consumption. - Merged on Magento 2.4.7

- Use the [Quality Patch Tool](https://experienceleague.adobe.com/tools/commerce-quality-patches/index.html) to apply the patch **ACSD-53583** to Improve the partial reindex performance for "Category Products" and "Product Categories" indexers. - Merged on Magento 2.4.7 - Do not install if you have Live Search! For merchants withLiveSearch, please apply [ACSD-55719](https://jira.corp.adobe.com/browse/ACSD-55719 "[Magento Cloud] Deployment fails when applying patch ACSD-53583") (Up to Magento 2.4.3-p3) - Not available in the quality patch

- Use the [Quality Patch Tool](https://experienceleague.adobe.com/tools/commerce-quality-patches/index.html) to apply the patch **ACSD-56415** to fix an issue where the performance of the partial price indexing is slowed down due to a DELETE query when the database has a lot of partial price data to index. - Merged on Magento 2.4.7

- Apply the patch [**ACSD-56226_2.4.6-p2.patch](file:///C:/download/attachments/3234969246/ACSD-56226_2.4.6-p2.patch?version=2&modificationDate=1721067391690&api=v2)** attached to fix an issue that was causing performance degradation with synchronous replication, which should be set to **true** to ensure data consistency. - Merged on Magento 2.4.7

- Use the [Quality Patch Tool](https://experienceleague.adobe.com/tools/commerce-quality-patches/index.html) to apply the patch **ACSD-58739** to convert temp tables to physical tables during the reindex. This will reduce memory usage, prevent issues with resource contention, and prevent errors like “Base table or view not found.” - This is for Magento 2.4.7+ only.

- Apply the patch[**ACSD-60549_2.4.4-p8.patch**](file:///C:/download/attachments/3234969246/ACSD-60549_2.4.4-p8.patch?version=1&modificationDate=1733142277247&api=v2) to optimize indexing operations by introducing a No DDL Mode. This significantly reduces database load and prevents node desynchronization by avoiding table recreations during reindexing, using efficient table switching instead. This patch is particularly effective for environments experiencing indexer-related slowdowns, deadlocks, or database node sync issues. - This is for Magento 2.4.4 only

- Use the [Quality Patch Tool](https://experienceleague.adobe.com/tools/commerce-quality-patches/index.html) to apply patch **ACSD-62577** to optimize search query performance by restructuring database indexes and improving SQL queries. The patch disables inefficient indexes and adds new compound indexes that include num\_results and popularity columns, while also improving query specificity with proper table prefixes. - This is for all Magento versions, for merchants with slowness on search page.

#### Soft purge on Fastly
